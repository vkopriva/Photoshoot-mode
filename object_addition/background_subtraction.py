...
*INCLUDES clustering for better isolation of objects*
...
def cluster_foreground(foreground: np.ndarray, eps: float = 0.3, min_samples: int = 3, min_cluster_size: int = 50):
    """
    Cluster foreground points with DBSCAN and return valid clusters.

    Returns:
        List of (label, points) tuples for clusters with >= min_cluster_size points.
    """
    if len(foreground) == 0:
        return []
    clustering = DBSCAN(eps=eps, min_samples=min_samples).fit(foreground)
    labels = clustering.labels_
    unique_labels = sorted(set(labels) - {-1})
    clusters = []
    for lbl in unique_labels:
        pts = foreground[labels == lbl]
        if len(pts) >= min_cluster_size:
            clusters.append((lbl, pts))
    return clusters
